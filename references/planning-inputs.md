...
The selected current quote defaults to DEX Screener with disclosed GeckoTerminal availability fallback; respect explicit `source` selection and set `cross_check` only on request. Fallback must not bypass access, schema or identity failures. Surface fallback reasons and material cross-check differences before proposing assumptions. Use only the selected provider's response for mapping or a provider-implied USD/ETH relation; never mix denominations, average sources or prefer the higher price. Retain each provider's separate retrieval time and unknown quote-observation time. Cross-checking does not approve any future economics or replace a supplied price.
...
The additive supply/restriction observations in [inspection](inspection.md#supply-restrictions-and-control-context) are context, not new schema-1 economic inputs. `token_burned_forever` and `token_ledger_retired` decompose `permanent_removed`; neither that sum nor arithmetic ceiling headroom replaces `remaining_gross_budget`. Separate `launch_holding_cap_enabled`, `launch_holding_cap_active` and `launch_schedule_active`; a cap, flag, binding or pending owner does not prove a future acquisition, transfer or exit can succeed. Preserve these observations outside scenario JSON and do not bypass unavailable inventory or execution evidence.

The publisher says accrual stops at epoch end until rollover (`sr-protocol-conditions`). `epoch_start`/`epoch_end`, emissions status and a stream-rate getter do not prove completed settlement or uninterrupted future accrual. The engine has no rollover scheduling, settlement-pending input or delay model; do not silently treat a daily extrapolation as a settlement-aware forecast.
