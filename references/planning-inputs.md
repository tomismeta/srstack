...
| Base issuance and multiplier | The displayed “Base issuance rate” is described as **policy-scaled**; the engine multiplies `base_daily_issuance` again. Do not map the label directly or double-apply the multiplier. Require an established unscaled global daily basis; current streamed rate also includes recycled fees. Pre-launch zero streaming is not a future zero assumption. |
| Remaining issuance budget | Token supply, max supply, burned value and accrued balances are different quantities. Do not compute the engine's remaining gross issuance budget as max supply minus minted supply without establishing all accounting semantics. A hard cap is not observed remaining budget. |
...
| Withdrawal fee | A marginal estimate is not a size-specific final quote or constant fee over the scenario. “Not started” is missing, not zero. Keep the engine's fixed percentage explicitly hypothetical. |
