...
Never self-update or rewrite installed rules/catalogs. New observations belong in the response with provenance, not cached values or observation/holdings files. Failures, missing history, redactions and empty provider responses are gaps, not zero activity or invented facts. DEX Screener prices are provider-reported indicative marks: retrieval time supplies neither independent freshness assurance nor a chain block, and unknown quote-observation time stays unknown. Gross accrued ledger value is not wallet value, net proceeds or charter resale/earning capacity. Inspection is on demand: no background polling, schedules or alerts.
