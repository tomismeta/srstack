...
A lower assumed issuance rate can be an explicitly approved sensitivity, not an exact correction for epoch downtime. Do not silently haircut inputs, shorten the scenario horizon or switch to stress mode; a pause affects accrual timing and purchase opportunities that a single rate adjustment does not reproduce. See [planner limits](planning-model.md#published-mechanics-versus-fixed-proxies).
