...
For current auction-day length, availability and usable prices, use `scripts/snapshot.py auctions`. Summary excludes last-sale/closing getters; full detail marks them `not_historical`. For past purchases and per-round accounting use [bounded auction history](auction-history.md), not current state or an announcement.
