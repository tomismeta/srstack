...
Use **[Robinhood Etherscan](https://robin.etherscan.io/)** and each catalog entry's explorer link for fresh source/ABI publication and verification lookups. Match chain plus full address; Ethereum `etherscan.io` is chain 1. A similar-match label alone does not authenticate this deployment's semantics. If the lookup or read is unavailable, report the current result as unavailable; an earlier saved observation may be described only with its original time and scope, not as a fresh verdict.
