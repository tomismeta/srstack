...
`founding-whitelist-window-control` records that source policy. Exact scheduled clock times and the public opening price are not established from current source; current claims require fresh evidence, while hypothetical schedules/prices must be labeled assumptions.
