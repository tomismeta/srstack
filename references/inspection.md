...
Daily auction `currentPrice()` is only a returned curve value until start/pause/inventory observations support availability. It can keep decaying after sellout. The helper omits normalized current prices when inactive, paused, sold out or status is unknown, while full evidence may retain raw getter values. Never relabel those raw values as purchasable quotes. `lastSalePrice()` is historical; derived closing price requires a last-sale day matching the current sold-out day. The fixed reader provides neither a STANDARD/ETH market quote nor a guarantee that an observed auction purchase would succeed.
...
Each helper invocation has its own block anchor. Separate protocol/auction/charter invocations may use different blocks; never describe a combined answer as one atomic snapshot unless the returned anchors actually match. Retain those details internally and mention timing differences only when material.
