...
Ask only for missing user-dependent scope; charter reads need a public charter ID, never a wallet scan. Helpers fix the network, targets and units. Use one narrowest view covering the request, default `summary`; `full` is for requested raw evidence, not a richer financial answer.
...
Use only the selected provider's returned denominations. A partial quote leaves the missing denomination unavailable: no currency substitution, cross-provider fill, averaging or higher-price selection. Default DEX Screener permits only the helper's labelled availability fallback to GeckoTerminal; never change providers after access denial, malformed/oversized data or identity mismatch. Honour explicit `--source dexscreener|geckoterminal`; add `--cross-check` only if requested. `--quote` conflicts with `--amount-standard`; no flags means JSON stdin, not an implicit quote.
