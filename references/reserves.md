...
For observed routing/liability and vault controls use `snapshot.py treasury`. An explicitly specified `--asset ADDRESS` is checked with `isReserveAsset` before holdings/pool calls; approved holdings remain raw token units without independently authenticated decimals, not a portfolio or custody assurance. `history.py buybacks` reports bounded `BuybackExecuted` ETH/STANDARD accounting, not aggregate burns or independently reconciled transfers. [Inspection](inspection.md#common-question-paths) preserves the distinction between current settings, queued shares and effective limits.

The publisher-linked ABIs contain successor holdings-migration entries for both vaults and FeeSplitter. Asset movement to a successor is distinct from changing deployed code; whitepaper non-upgradeability language is not a blanket proof that holdings can never move between components. The ABI alone establishes neither deployment correspondence nor who can move what, registry replacement rules or present authority. Keep these as implementation questions, not custody assurances; requested migration explanations or unsigned preparation follow [execution](execution.md), without signing or submission. [sr-contract-directory; contracts](contracts.md)

The [Second Mandate](updates.md#second-mandate-liquidity-for-tokenized-stocks) proposes seeding tokenized-stock markets, coordinating external capital and returning trading fees to the Reserve for further markets. It establishes neither a new ongoing ETH split nor deployed positions or holder revenue rights. Its reserve-position panel is explicitly sample content; the separate “as of today” graphic uses fixed values in the reviewed component, not live reserve observations. Fresh retrieval alone does not establish holdings. Keep this announced strategy separate from whitepaper reserve accounting and the developer POL proposal. [sr-second-mandate-manifesto]
