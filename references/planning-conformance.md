...
| `scenarios.<index>.multiplier` | ≥ minimum and ≤ maximum | `frontend-policy-range` | `multiplier range displayed by protocol page`; source kind is a frontend publication, status `observed frontend publication, not deployed configuration`. |

The current frontend range is checked as a **publication only**, not an authenticated on-chain bound, current multiplier, or verified policy rule. The report retains the canonical value, unit, status, source IDs, and locator so this weaker evidence is not presented as contract configuration. Known constraint values must be finite and nonnegative; caps must be integral, the branch maximum positive, and the frontend minimum no greater than its maximum.

Unresolved source semantics are not missing user inputs. The user still supplies explicit assumptions for `base_daily_issuance` (`base-issuance`), `license_cost_tokens` (`license-floor-formula`), scenario `resolution_fee_pct` (`resolution-fee-formula`), `sale_fee_pct` (`trading-fee`), and scenario `sale_tax_pct` (`launch-trading-tax-curve`). Constant scenario multipliers remain policy proxies linked to `multiplier-update-rule`, even inside the published frontend range. Credits funding remains unresolved payment feasibility, not a verified way to buy licenses. `entry_cost_eth` remains an unresolved acquisition-context assumption: `whitelist-liquidity-fee` is context for a specific mint route, not a universal entry price or an enforced generic cost.

Nullable or otherwise unknown source records remain unknown; do not replace supplied assumptions with null, infer a fee or policy rule, or automatically apply a future newly populated record without an explicitly reviewed mapping. Other branch growth and global auction inventory, continuous policy/accrual approximated by daily steps, fee redistribution, dormancy, credit-payment feasibility, fee/tax bases, sequential haircuts, and absent residual valuation remain explicit unchecked model assumptions. Passing the supported checks does not establish that any or all scenarios are protocol-feasible.
