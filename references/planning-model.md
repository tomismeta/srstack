...
This ordering and fee basis are **model assumptions**, not an authenticated protocol tax formula, a trading simulation, or a guarantee of liquidity or executable price. Exit gas is charged once only when `k > 0`. Total cash outlay is entry cost + entry gas + expansion cash (including license gas) + charged exit gas. `net_cash_eth` is recovered ETH minus that outlay. `delta_vs_keep_eth` compares this cash result with keep under the same scenario and exit mode.
...
Summary omits purchase records, daily history and per-fee drilldown, not conformance or evidence boundaries. Top-level limitations retain partial checks; unknown fee, policy, auction and credits semantics; absent residual valuation; and the lack of quotes or contract verification.
