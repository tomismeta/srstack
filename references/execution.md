...
Use with [inspection](inspection.md) and [safety](safety.md). Answer first; no mandatory opening warning or integrity trace. A factual snapshot is an RPC observation using a publisher ABI; a market quote is provider-reported indicative pricing.

Three fixed entrypoints use existing permitted Python 3.10+: `scripts/snapshot.py` for bounded public RPC reads, `scripts/price.py` for canonical-pool prices and optional gross amount valuation, and `scripts/verify.py` for package verification and explicitly selected live checks. The two data helpers support JSON stdin/stdout and fixed CLI conveniences; the diagnostic takes only its documented flags and emits compact JSON. None accepts arbitrary code, paths, endpoints, selectors, addresses, headers or wallet inputs. Reads do not unlock transactions, token purchases, gas estimation or state-changing simulations.

Resolve the installed root from a trusted reviewed package/commit, not a working-directory lookalike or website. Before executing a helper, use existing host read/hash utilities or minimal fixed launcher glue to compare its script and fixed data files with the trusted `release-manifest.json` as data. Snapshot requires exactly `assets/entities/robinhood.json` and `assets/interfaces/robinhood-reads.json`; price requires only `assets/entities/robinhood.json`, not the interface catalog. Verification paths must remain inside that root and are never caller-selected. A self-supplied matching manifest alone does not establish trust or source authenticity. Failed/unavailable verification stops execution. Keep hashes and resource bodies out of chat unless requested; a concise integrity blocker is enough.
...
For the data helpers, no arguments means JSON stdin; any non-help CLI arguments select explicit CLI mode and **never read, merge or fall back to stdin**. `--help` documents both interfaces without a live read. Use exact, separate flag/value tokens (not `--flag=value`); unknown, repeated, abbreviated or conflicting flags are invalid input before any network access. Snapshot's positional view comes first. CLI configs flow through the existing validators and readers; structured error and exit conventions remain unchanged. CLI values are data: use separate argument-array elements, not interpolated shell syntax. Amount arguments can appear in process listings or host logs; stdin is still available and does not imply transcript privacy.
...
Identity comes only from fixed `assets/entities/robinhood.json`: `sr-robinhood-standard`'s address and `market.pool_id`, Robinhood chain 4663 and native ETH's zero address. Both providers use fixed direct HTTPS GETs, no search, token-pair discovery or legacy-pair-alias fallback, and `User-Agent: srstack/0.2.0 (+https://github.com/tomismeta/srstack)`. The optional amount is multiplied locally; neither amount nor charter ID is sent to a provider.
...
Normal diagnostic stdout is compact JSON: `schema_version: 1`, root `status: "ok"|"failed"`, `stages` and an integrity-not-authenticity `note`. Only integrity and selected stages appear. Each stage has `name` (`integrity`, `charter` or `price`), `status` (`ok`, `partial`, `failed` or `skipped`), `scope` and `elapsed_ms`; failures may add a static `error` code or `helper_exit`. A nonzero helper exit retains its structured `helper_error` type and a printable message capped at 240 characters, not raw stderr. Partial results include `helper_error_count`, up to eight `helper_errors` and `helper_errors_truncated`; use the direct helper for complete details. This distinguishes reported transport, chain and data failures without treating every exit 5 as a network outage. Successful integrity adds `files` (including the manifest), `bytes` and `content_sha256`. It does not print full financial results or child output.
