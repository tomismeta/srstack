...
Price reads default to DEX Screener, with a disclosed GeckoTerminal fallback only for availability failures—not access denial, invalid data or identity mismatch. Honour explicit provider selection (“Use GeckoTerminal”); cross-check the other provider only when requested (“Cross-check the current price”). Keep selected-provider quotes and gross values together; do not average, choose the higher quote or fill missing denominations across providers. Show fallback and material cross-check differences; each provider's retrieval time is separate and quote age remains unknown. See the [price contract](references/planning-execution.md#price-helper).
